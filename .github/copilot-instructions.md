...
Windows 環境では、`gh` CLI の JSON 出力を PowerShell の `ConvertFrom-Json` でパースすると日本語が文字化けすることがある。
代わりに **Python** を使って JSON をパースする。

```bash
# NG: ConvertFrom-Json は日本語で文字化けが起きることがある
gh api graphql -f query='...' | ConvertFrom-Json

# OK: Python でパースする
gh api graphql -f query='...' | python -c "import sys,json; data=json.load(sys.stdin); ..."

# OK: 一度ファイルに保存してから Python で読み込む（パイプが複雑なとき）
gh api graphql -f query='...' > tmp.json
python -c "
import json
